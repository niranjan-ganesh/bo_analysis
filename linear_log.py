...
# Perform log transformation on Views
X[:, 0] = np.log1p(X[:, 0])
...
print(f'Equation: BO Collection = {intercept:.2f} + {coefficients[0]:.2f} * ln(Views) + {coefficients[1]:.2f} * Sentiment')
equation = np.array([f'BO Collection (₹) = {intercept:.2f} + {coefficients[0]:.2f} * ln(Views) + {coefficients[1]:.2f} * Sentiment'])
...
plt.scatter(X_test[:, 0], y_pred, color='red', linewidth=3, label='Predicted Data')
plt.xlabel('ln(Views)')
plt.ylabel('Box Office Collection (in ₹ 10 billion)')
plt.title('Linear-Log Regression: ln(Views) vs. Box Office Collection')
